...
librerias <- c("BiocManager", "dplyr","rsconnect", "shiny",
               ...
               "ggpubr","dendextend", "MASS", "plotly",
               "knitr", "mixOmics", "FactoMineR",
               "factoextra","DT", "shinyWidgets",
               "ggpubr","plyr")
...
1. Four analyzes [@gu_principal_2011; @le_cao_sparse_2011; @ruiz-perez_so_2020]:
   ...
   + **Volcano Plot**: is a type of scatterplot used to quickly identify changes in large data sets made up of replicated data. Represents the relationship between the magnitude of change (fold change) and statistical significance (p.value).
...
c18 <- read.table("C18_Negative.csv",
                  sep=",",
                  header = TRUE)
names(c18) <- gsub("X","", names(c18))


clinical <- read.table("Dados_Clinicos.csv",
                  sep=",",
                  header = TRUE)
...
wellPanel(
      ...
      selectInput("dataset", "Data Examples:",
...
set  <- reactive({
  ...
  aux3 <- data()[,names(data()) %in% aux2$Features]
...
tabsetPanel(type = "tabs",
               tabPanel("PCA", 
                        plotlyOutput("pca")),
               tabPanel("PLS-DA",plotlyOutput("plsda")),
...
output$clustering <- renderPlot({
  ...
  ggplot(clustering.exp()$segments) +
